...
- **`pin_role_groups`** (`list[str]`，默认 `[]`)：标注权。从 `target_role_groups` 中选出的组 ID 子集，这些组的角色在工单频道中额外获得 Discord 的「标注消息」权限（PIN_MESSAGES，2025-08 从「管理消息」拆分出的独立权限），可以在工单讨论中置顶/标注关键消息。引用不在 `target_role_groups` 中的组没有意义（有标注权却看不到频道）。
